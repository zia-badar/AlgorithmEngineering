...
//						&& cg->all_nodes[i].connected_nodes.find(node_weight_pair(j, 0))
//							== cg->all_nodes[i].connected_nodes.end())
//						int i = 1 / 0;
//					if (!(cg->all_edge_statuses[i][j] & cg->CONNECTION_CONNECTED)
//						&& cg->all_nodes[i].disconnected_nodes.find(node_weight_pair(j, 0))
//							== cg->all_nodes[i].disconnected_nodes.end())
//						int i = 1 / 0;
//					changed_edges_cost += (cg->all_edge_statuses[i][j] & cg->CONNECTION_CONNECTED) ?
//						abs(cg->all_nodes[i].connected_nodes.find(node_weight_pair(j, 0))->weight) :
//						abs(cg->all_nodes[i].disconnected_nodes.find(node_weight_pair(j, 0))->weight);
//				}
//		}
